...
exports.addCourse = asyncHandler(async (req, res, next) => {
  ...
  if (bootcamp.user.toString() !== req.user._id && req.user.role !== "admin") {
    return next(
      new errorResponse(
        `User is not authorized to add course on this bootcamp`,
        401
      )
    );
  }
...
exports.updateCourse = asyncHandler(async (req, res, next) => {
  ...
  if (course.user.toString() !== req.user._id && req.user.role !== "admin") {
    return next(
      new errorResponse(
        `User is not authorized to update this course on this bootcamp`,
        401
      )
    );
  }
...
exports.deleteCourse = asyncHandler(async (req, res, next) => {
  ...
  if (course.user.toString() !== req.user._id && req.user.role !== "admin") {
    return next(
      new errorResponse(
        `User is not authorized to delete this course on this bootcamp`,
        401
      )
    );
  }
